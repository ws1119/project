...
class TransH:
    ...
    def test_run(self, filter = False):
        ...
        f = open("result_torch/result.txt", 'a')
        f.write("epochs:" + str(epochs) + "\t" + "embedding_dim:" + str(embedding_dim) + "\t"
                + "lr:" + str(lr) + "\t" + "batch_size:" + str(batch_size)
                + "margin:" + str(margin)
                + "norm:" + str(norm)
                + "C:" + str(C) + "\n")
...
if __name__ == '__main__':
    ...
    batch_size = 50
    epochs = 300
    transH = TransH(entity_set, relation_set, triple_list,
                    embedding_dim=embedding_dim,
                    lr=lr,
                    margin=margin,
                    norm=norm,
                    C=C,
                    epsilon=epsilon, valid_triple_list = valid_triple_list)
    transH.data_initialise()
    transH.training_run(epochs=epochs, batch_size=batch_size,
                        out_file_title="result_torch/epoch{}_".format(epochs))

    file5 = ("result_torch/epoch{}_TransH_pytorch_entity_{}dim_batch{}"
             .format(epochs, embedding_dim, batch_size))
    file6 = ("result_torch/epoch{}_TransH_pytorch_norm_relations_{}dim_batch{}"
             .format(epochs, embedding_dim, batch_size))
    file7 = ("result_torch/epoch{}_TransH_pytorch_hyper_relations_{}dim_batch{}"
             .format(epochs, embedding_dim, batch_size))
    file8 = "data/test.txt"
    file9 = "result_torch/epoch{}_loss_record.txt".format(epochs)
    transH.insert_data(file5, file6, file7, file8, file9)
    transH.test_run(filter = False)
